...
async fn handle_reader(mut reader: OwnedReadHalf, sender: Sender, room_sender: mpsc::Sender<(Command, Sender)>) {
...
async fn handle_connection(stream: TcpStream, room_sender: RoomSender) {
    ...
    tokio::spawn(async move { handle_reader(reader, sender, room_sender).await });
}
